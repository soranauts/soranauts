...
|-----------|--------|----------------|----------------|-------------|
| **Rust** | Excellent | Compile-time | Steep | **[Polkadot](/polkadot-vs-ethereum-a-comparative-analysis)**, **Solana**, **SORA** |
...
For a deeper comparison of execution environments, see [Polkadot (WASM / Rust) vs Ethereum (EVM / Solidity)](/wasm-rust-substrate-vs-evm-solidity-ethereum).
...
| **Cross-chain systems** | Deterministic VMs, cross-chain bridges | **SORA Hub Chain (v3)** |
...
- **WASM smart contract execution** (Polkadot/Substrate ecosystem)  

SORA teams have historically used **Hyperledger Iroha 2** to prototype Hub Chain concepts.  
As of December 2025, **[SORA v3 (Nexus)](/sora-nexus-complete-guide)** is being built on **Hyperledger Iroha 3**, designed as a politically neutral **Hub Chain** enabling secure, controllable cross-network interoperability.
...
Rust's strict semantics and lack of runtime unpredictability make it a natural fit for sovereign and institutional blockchain systems. **SORA v3 (Nexus)**, built on Hyperledger Iroha 3, uses Rust extensively for its deterministic IVM (Iroha Virtual Machine) and Kotodama runtime.
...
<FaqSection>
  ...
  <details>
    ...
    **SORA v2** runs on Substrate (which supports WASM), while **SORA v3 (Nexus)** is built on Hyperledger Iroha 3 using the IVM (Iroha Virtual Machine) with Kotodama bytecode — a purpose-built deterministic runtime, not WASM. Rust powers economic modules, governance logic, and Polkaswap's DeFi infrastructure across both versions.
  </details>
