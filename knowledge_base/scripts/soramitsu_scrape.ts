...
function sanitizeAndNormalizeHtml(rawHtml: string): string {
  // First sanitize with sanitize-html
...
async function logRun(event: Record<string, any>): Promise<void> {
  ...
  const runlogPath = path.join(runlogDir, `soramitsu-${Date.now()}.jsonl`);
...
async function fetchPage(url: string, state: Map<string, StateEntry>, robots: robotsParser.Robot | null): Promise<{ status: number; etag?: string; lastModified?: string; html?: string; error?: boolean }> {
  ...
  try {
    // Try HEAD first for 404 detection
    let r: any;
    try {
    ...
    // Get full body if HEAD was used
    if (!('body' in r) || !r.body) {
      r = await got(url, {
        headers: { 'User-Agent': env.USER_AGENT },
        timeout: { request: env.REQUEST_TIMEOUT },
...
(async () => {
  ...
  console.log('Scraping SORAMITSU site...');
  if (options.dryRun) console.log('  [DRY RUN MODE]');
  
  const state = await loadState();
  const domains = env.CRAWL_DOMAINS.split(',').map(d => d.trim()).filter(Boolean);
  if (domains.length === 0) {
    throw new Error('No crawl domains configured. Set CRAWL_DOMAINS in env.');
  }
  ...
  // Seed from sitemaps for each domain (depth=1)
  ...
  for (const domain of domains) {
    ...
    if (sitemapUrls.size && domain === primaryDomain) {
      // keep primary sitemap results but continue looping to gather others
      continue;
    }
  }
  ...
  // Also load allowlist
  const allowlistPath = path.join(env.KB_DIR, 'scripts', 'config', 'soramitsu.allowlist.txt');
  ...
  // Combine and dedupe URLs (limit depth to 1 from sitemap)
  const urls = new Set<string>([...allowlist]);
  const disallowPatterns = [/\/news\/?$/, /\/solutions\/?$/];
  for (const url of sitemapUrls) {
    if (disallowPatterns.some(pattern => pattern.test(url))) {
      continue; // Skip known 404 endpoints
    }
    const parsed = parseUrl(url);
  ...
  for (const url of urls) {
    ...
    // Normalize markdown for consistent hashing (same as ingest uses)
    const normalized = md
      .replace(/\r\n?/g, '\n')
      .replace(/[ \t]+/g, ' ')
      .replace(/\n{3,}/g, '\n\n')
