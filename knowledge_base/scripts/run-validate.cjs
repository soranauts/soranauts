#!/usr/bin/env node
// Wrapper script to run validate-kb.ts with proper module resolution
// Note: This script has known module resolution issues with tsx/pnpm locally
// It works correctly in CI. For local execution, use: pnpm --filter @soranauts/web exec tsx knowledge_base/scripts/validate-kb.ts
const { spawn } = require('child_process');
const path = require('path');

const scriptPath = path.join(__dirname, 'validate-kb.ts');
